...
> "us-east-2 tiene la base de datos replicando en tiempo real, pero no tiene servidores. El código para crearlos existe, con count=0. Eso es exactamente el patrón Pilot Light: mínimo costo, máxima preparación."
...
**Mientras esperas, puedes comentar con la audiencia:**
> "En un escenario real este momento representaría una caída de región, un incidente de seguridad o una corrupción de datos. El equipo de operaciones habría recibido una alerta y estaría ejecutando el runbook de DR."

> "Noten que la base de datos en us-east-2 sigue replicando normalmente. No se ha visto afectada. Eso es la ventaja del Pilot Light: la capa de datos es independiente de la capa de cómputo."
...
**Qué mostrar en la consola AWS us-east-2 → RDS:**
- El estado cambia **available → modifying → available** (tarda 2-5 minutos)
- Cuando vuelva a **available**, el campo "Replication role" cambia de `Replica` a `Primary`

**Mientras esperas, puedes comentar con la audiencia:**
> "La promoción es un proceso de RDS que convierte la réplica en una instancia primaria independiente. A partir de este momento acepta escrituras. No hay pérdida de datos porque la replicación era continua."

> "Esto es lo que diferencia Pilot Light de un backup tradicional. Con un backup tendrías que restaurar los datos, lo que podría tomar horas. Aquí la base de datos ya estaba lista y sincronizada."
...
Muestra el plan a la audiencia:
```
Plan: 1 to add, 0 to change, 0 to destroy.
```

**Mientras corre el apply (~2-3 minutos), puedes comentar:**
...
> "En un entorno productivo este apply lo dispararía un pipeline automatizado, no una persona. El cambio de variable sería la señal de activación."
...
**Mientras esperas, puedes comentar:**
> "El EC2 acaba de arrancar y está ejecutando el script de user_data: instalando nginx y generando la página HTML con los datos de la región. En producción aquí estaría desplegando tu aplicación real desde un artefacto."
...
Crea el health check del EC2 DR en Route 53:
- Route 53 → Health checks → Create health check
- Name: `dr-pilot-light-dr-hc`, IP: la IP del EC2 DR, mismo resto de configuración
- En ~30 segundos aparece en **Healthy**

**Checklist visual final para la audiencia:**
...
**Mensaje de cierre:**
> "En menos de 15 minutos pasamos de una región caída a tener la aplicación respondiendo desde la región DR con la base de datos activa y aceptando escrituras. Ese es el RTO real del patrón Pilot Light. Sin intervención manual en la base de datos, sin restaurar backups, sin reconfigurar redes."

---

## Al terminar el webinar

Elimina los health checks de Route 53 manualmente antes de destruir (Route 53 → Health checks → seleccionar → Delete).
...
Escribe `DESTRUIR` cuando lo pida y confirma con `s`.
